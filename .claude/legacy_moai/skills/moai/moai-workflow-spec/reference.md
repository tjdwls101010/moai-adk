...
## Enhanced SPEC File Structure (4-File Standard)

### 권장 디렉토리 구조

MoAI-ADK는 GitHub Spec-Kit과 AWS Kiro IDE의 업계 표준을 참고하여 다음 4파일 구조를 권장한다:

```
.moai/specs/SPEC-XXX/
├── spec.md          # EARS 요구사항 명세 (핵심)
├── design.md        # 기술 설계 문서 (신규)
├── tasks.md         # 구현 작업 목록 (신규)
└── acceptance.md    # Gherkin 수용 기준
```
...
| 파일 | 역할 | 주요 내용 | 참고 표준 |
|-----|-----|---------|----------|
| spec.md | 요구사항 정의 | EARS 5패턴, 제약사항 | Kiro requirements.md |
| design.md | 기술 설계 | 아키텍처, 시퀀스 다이어그램, API 설계 | Kiro design.md |
| tasks.md | 구현 계획 | 체크리스트 형식 작업 목록 | Spec-Kit tasks.md |
| acceptance.md | 수용 기준 | Gherkin Given-When-Then | MoAI 고유 |

### Template 4: design.md (기술 설계)

```markdown
# SPEC-XXX: [Feature Name] - Technical Design
...
### System Context
[시스템이 외부 컴포넌트와 어떻게 상호작용하는지 설명]

### Component Diagram
```mermaid
graph TB
    subgraph System["Feature Boundary"]
        A[Component A]
        B[Component B]
    end
...
### Template 5: tasks.md (구현 작업 목록)

```markdown
# SPEC-XXX: [Feature Name] - Implementation Tasks
...
- [구현 중 발견된 이슈나 결정 사항 기록]
```

### 마이그레이션 가이드

기존 3파일 구조에서 4파일 구조로 마이그레이션하는 방법:

1. 기존 plan.md의 기술 설계 부분을 design.md로 분리
2. 기존 plan.md의 작업 목록을 tasks.md로 분리
3. 기존 spec.md와 acceptance.md는 유지
4. plan.md는 deprecated 처리하거나 design.md + tasks.md 요약본으로 유지
